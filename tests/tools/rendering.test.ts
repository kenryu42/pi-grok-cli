...
describe("tool rendering helpers", () => {
	...
	it("renders summaries, expanded text, missing text fallback, and partial state", () => {
		...
		expect(renderText(renderResultText(result, false, "summary"))).toBe(
			"summary",
		);
		expect(renderText(renderResultText(result, true, "summary"))).toBe(
			"full output",
		);
		expect(
			renderText(
				renderResultText(
					{ content: [{ type: "image" }], details: {} },
					true,
					"summary",
				),
			),
		...
		expect(
			renderText(renderResultSummary(result, false, true, "summary")),
		).toBe("Running...");
	});
